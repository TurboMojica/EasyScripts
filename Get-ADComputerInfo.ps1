...
Get-ADComputer -SearchBase $OU -Filter * -Properties info | Select name,{$_.info.split(';')[0]},{$_.info.split(';')[1]},{$_.info.split(';')[3]},{$_.info.split(';')[5]} | Export-csv -Force -NoTypeInformation $ExportLocation\ComputerList.csv

Write-Host "`n`nReport Complete"
